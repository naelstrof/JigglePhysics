...
>>>>>>> Stashed changes
        ...
        public void UpdateParameters(JiggleTree tree, List<JigglePointParameters> parameters)
        {
            ...
            {
                var bone = bones[i];
                var cache = GetCache(bone.GetInstanceID());// <- this could become just a array lookup instead of using a dictonary.
                parameters.Add(GetJiggleBoneParameter(cache.normalizedDistanceFromRoot));//could normalizedDistanceFromRoot just be outside of the cache (even faster?)
            }
            tree.SetParameters(parameters);
        }
